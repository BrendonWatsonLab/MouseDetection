...
class ActigraphyProcessor:
    ...
    def set_processing_parameters(self, min_size_threshold):
    ...
    def process_single_video_file(self, video_file, name_stamp, roi, output_directory, progress_callback):
        ...
        min_duration_ms = 2000  # Minimum duration the mouse should be present in the ROI
        ...
        while True:
            ...
            # sees if mouse is detected
            detected = self.detect_mouse(roi_frame, self.min_size_threshold)
            posix_time = int(creation_time + (elapsed_millis))

            print(f"Frame: {frame_number}, Mouse Detected: {detected}, Current Time: {posix_time}")

            # logic for writing start and end time
            if detected and not is_rat_present:
                is_rat_present = True
                detection_start_time = posix_time
                print(f"Detection Start: {detection_start_time}")  # Debug print
            elif not detected and is_rat_present:
                is_rat_present = False
                detection_end_time = posix_time
                result_rows.append((detection_start_time, detection_end_time))
                print(f"Detection End: {detection_end_time}")  # Debug print
        ...
        # handle the case where the mouse is still present at the end of the video
        if is_rat_present:
            detection_end_time = cap.get(cv2.CAP_PROP_POS_MSEC)
            result_rows.append((detection_start_time, int(creation_time + detection_end_time)))
    ...
    def detect_mouse(self, frame, min_size_threshold):
        # ensures frame is in grayscale
        if len(frame.shape) == 3 and frame.shape[2] == 3:
            frame_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        else:
            frame_gray = frame

        _, thresholded = cv2.threshold(frame_gray, 50, 255, cv2.THRESH_BINARY_INV)
